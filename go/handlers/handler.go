...
func AllowlistHandler(w http.ResponseWriter, r *http.Request) {
	...
	password := "123456" // For your real implementation fetch your password
	privateKey, err := selfsign.LoadPrivateKey("keys", password)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	respOK, respErr, err := request.CallAPI(requestBody)
	if err != nil {
	...
	if !respOK.IsEmpty() {
		cert, err := selfsign.LoadPFXCertificate("keys", password)
		if err != nil {
